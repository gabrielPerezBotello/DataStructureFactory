...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	/** Field for holding the actual tree object we will be manipulating. */
	 ...
	 * 
	 * @param compare a Comparator to be used for comparing nodes in the tree
	 */
	...
	protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
		...
		public void rotate(Position<Entry<K, V>> p) {
			Position<Entry<K, V>> node = validate(p);
			Position<Entry<K, V>> parent = validate(parent(node));
			

			if (parent(parent) == null) {
				setRoot(node);
				validate(node).setParent(null);
			} else {
				Position<Entry<K, V>> grandparent = validate(parent(parent));
				
				if (parent == left(grandparent)) {
					relink(validate(grandparent), validate(node), true);
				} else {
					relink(validate(grandparent), validate(node), false);
				}
			}

			if (node == left(parent)) {
				relink(validate(parent), validate(right(node)), true);
				relink(validate(node), validate(parent), false);
			} else {
				relink(validate(parent), validate(left(node)), false);
				relink(validate(node), validate(parent), true);
			}
		...
		public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
			...
			if (node == validate(left(parent)) && parent == validate(left(grandparent))
					|| node == validate(right(parent)) && parent == validate(right(grandparent))) {
				rotate(parent);
				return parent;
			} else {
				rotate(node);
		...
		protected static class BSTNode<E> extends BinaryTreeNode<E> {
			/** Property field to aid in implementing binary search behavior. */
	...
	/////////////////////////////////////////////////////////////////////////////
	/**
	 * Root method for BalanceableBinaryTree class, that returns root node.
	 * 
	 * @return Position object representing root of tree
	 */
	...
	/**
	 * Returns the parent of a position node given in parameters.
	 * 
	 * @param p the node we will be finding the parent of
	 * @return Position object representing the parent of p
	 */
	...
	/**
	 * Checks if p is an internal node.
	 * 
	 * @param p the node we are checking
	 * @return true if p is internal, otherwise false
	 */
	...
	/**
	 * Sets p to have the given entry.
	 * 
	 * @param p     the node we will be changing the entry of
	 * @param entry the entry we will replace the old one
	 * @return entry the old entry that was originally in taht position
	 */
	...
	/**
	 * Checks if p is a leaf node.
	 * 
	 * @param p the position we are checking
	 * @return true if p is a leaf node, otherwise false
	 */
	...
	/**
	 * Checks if p is the root of the tree.
	 * 
	 * @param p the position we are checking
	 * @return true if p is the root, otherwise false
	 */
	 ...
	 * 
	 * @param p the node that we will return the left child of
	 ...
	 * 
	 * @param p the node we will return the right child of
	...
	/**
	 * Returns the sibling of p.
	 * 
	 * @param p the node that we will return the sibling of
