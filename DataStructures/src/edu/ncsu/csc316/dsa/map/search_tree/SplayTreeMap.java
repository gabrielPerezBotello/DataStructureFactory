...
public class SplayTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
	 ...
	 * 
	 * @param compare a comparator to be used to compare elements in the tree
	 */
	...
	private void splay(Position<Entry<K, V>> p) {
		...
		while (!isRoot(node)) {
			...
			if (parent(parent) == null) {
				// Zig
				rotate(node);
			} else if (node == left(parent) && parent == left(parent(parent))
					|| node == right(parent) && parent == right(parent(parent))) {
				// Zig-Zig
				rotate(parent); // rotate parent around grandparent
				rotate(node); // rotate node around parent
			} else {
				// Zig-Zag
				rotate(node); // rotate node around parent
				rotate(node); // rotate node around grandparent
			}
		}
	}
	 ...
	 * parent of p. MADE EDIT TO CREATE NODE POSITION AND USE THAT INSTEAD OF
	 * PARAMETER P.
	 */
