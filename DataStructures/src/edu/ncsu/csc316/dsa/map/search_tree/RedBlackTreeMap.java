...
public class RedBlackTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
	 ...
	 * 
	 * @param compare comparator used to compare elements in the tree
	 */
	...
	private void resolveRed(Position<Entry<K, V>> p) {
		...
		if (isRed(parent)) {
			...
			if (isBlack(uncle)) {
				// Case 1: the uncle is black (sibling of parent)
				Position<Entry<K, V>> middle = restructure(node);
				makeBlack(middle);
				makeRed(left(middle));
				makeRed(right(middle));
			} else {
				// Case 2: the uncle is red (sibling of parent)
				makeBlack(parent);
				makeBlack(uncle);
	...
	private void remedyDoubleBlack(Position<Entry<K, V>> p) {
		...
		if (isBlack(sibling)) {
			...
			if (isRed(left(sibling)) || isRed(right(sibling))) {
				Position<Entry<K, V>> temp = left(sibling);
				
				if (isRed(right(sibling))) {
					temp = right(sibling);
				}
