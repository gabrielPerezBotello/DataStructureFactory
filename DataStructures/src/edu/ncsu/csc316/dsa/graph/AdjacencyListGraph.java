...
public class AdjacencyListGraph<V, E> extends AbstractGraph<V, E> {
	/** Contains the list of all vertices in graph. */
    private PositionalList<Vertex<V>> vertexList;
    /** Contains the list of all edges in graph. */
    ...
    public Edge<E> insertEdge(Vertex<V> v1, Vertex<V> v2, E edgeData) {
        ...
        Position<Edge<E>> outPos = origin.getOutgoing().addLast(edge);
        Position<Edge<E>> inPos = destination.getIncoming().addLast(edge);
     ...
     * 
     * @param v the vertex being validated
     ...
     * 
     * @param e the edge being validated.
